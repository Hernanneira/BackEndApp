...
const mongooseMessages = mongoose.createConnection(
...
class Mensaje {
  mensajessDAO = mongooseMessages.model(
    "mensajes",
    require("../schemasModel/messagesSchema")
  );
  ...
  async getEmailMessages(email) {
    try {
      const content = await this.mensajessDAO.find({"email": email});
      return content;
    } catch (error) {
      return error;
    }
